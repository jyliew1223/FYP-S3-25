from django.urls import path

from . import views

from MyApp.Boundary.user_boundary import signup_view, get_user_view
from MyApp.Boundary.auth_boundary import (
    verify_id_token_view,
    verify_app_check_token_view,
)
from MyApp.Boundary.crag_info import (
    crag_info_view,
    crag_monthly_ranking_view,
    crag_trending_view,
)
from MyApp.Boundary.climb_logs import get_user_climb_logs_view
from MyApp.Boundary.climb_stats import get_user_climb_stats_view


urlpatterns = [
    path("", views.home, name="home"),
    path("signup/", signup_view, name="Sign Up User"),
    path("get_user/", get_user_view, name="Get User"),
    path("verify_id_token/", verify_id_token_view, name="Verify ID Token"),
    path(
        "verify_app_check_token/",
        verify_app_check_token_view,
        name="Verify App Check Token",
    ),
    path("crag_info/", crag_info_view, name="crag_info"),
    path(
        "crag_monthly_ranking/", crag_monthly_ranking_view, name="crag_monthly_ranking"
    ),
    path("crag_trending/", crag_trending_view, name="crag_trending"),
    path("climb_logs/", get_user_climb_logs_view, name="climb_logs"),
    path("climb_stats/", get_user_climb_stats_view, name="climb_stats"),
]
