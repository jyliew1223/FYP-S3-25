...
class CustomApiRequest {
    ...
    async sendRequest(ResponseClass = BaseApiResponse) {
        ...
        if (this.#attachAppCheckToken) {
            console.warn(
                `${this.constructor.name}: ByPassing App Check Token for Testing, Remember to implement if before deployment`
            );
        }
     ...
     */
    logResponse(res, _ResponseClass = BaseApiResponse, prefix = this.constructor.name) {
        if (!this.#response) return `${prefix}: Response is null.`;

        let log = `${prefix}: Response:\n`;
        log += `\t${"StatusCode".padEnd(12)}: ${res.status}\n`;
        log += `\t${"Success".padEnd(12)}: ${this.#response.success || "Success not stated"}\n`;
        log += `\t${"Message".padEnd(12)}: ${this.#response.message || "No message"}\n`;
        log += this.#logChildProperties(this.#response);
        log += `\t${"Errors".padEnd(12)}: ${this.#formatErrors(this.#response.errors)}\n`;
