...
@api_view(["GET"])
def search_posts_view(request: Request) -> Response:
    """
    Boundary: Handle HTTP request to search posts.
    
    Query Parameters:
        query: string (required) - Search query for post title, content, or tags
    ...
    try:
        posts = post_controller.search_posts(query, limit)
